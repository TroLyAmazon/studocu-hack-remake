window.addEventListener('load', function(){
    try {
        ...
        /* Luôn hiện 2 nút GitHub + Facebook ở vị trí cố định (góc phải, dưới nút Tải xuống) để không bị mất khi Studocu đổi layout/SPA */
        if (document.body && !document.getElementById("stuhack-version-bar")) {
            var bar = document.createElement("div");
            bar.id = "stuhack-version-bar";
            bar.appendChild(button_gh);
            bar.appendChild(button_fb);
...
const updateLogo = (logo) => {
    try {
        if (!logo || !logo.firstChild) return;
        const logoPath = logo.firstChild;
        if (typeof logoPath.innerHTML === 'undefined') return;
        logoPath.innerHTML = 
